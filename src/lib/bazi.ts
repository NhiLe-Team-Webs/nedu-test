...
export const getSolarTime = (isoDate: string, timezone: string = '+07:00') => {
  const date = toDate(isoDate);
  // Áp dụng timezone của người dùng
...
export const buildSixtyCycleObject = (sixtyCycle: SixtyCycle, me?: HeavenStem) => {
  ...
  return {
    天干: {
      ...
      十神: me.getName() === heavenStem.getName() ? undefined : me.getTenStar(heavenStem).toString(),
    },
    地支: {
...
export const buildBazi = (options: { solarTime: SolarTime; eightCharProviderSect?: 1 | 2; gender?: Gender }) => {
  const { solarTime, eightCharProviderSect = 2, gender = 1 } = options;
